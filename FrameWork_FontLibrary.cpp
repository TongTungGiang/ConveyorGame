...
void FontLibrary::fwf_renderText(const char* renderingString, std::string fontID, SDL_Color textColor, int x, int y)
{
	...
	SDL_Texture* textTexture = SDL_CreateTextureFromSurface(FrameWork::Window::instance()->fww_getWindowRenderer(), textSurface);
	SDL_FreeSurface(textSurface);

	SDL_Rect textRect;
	SDL_QueryTexture(textTexture, NULL, NULL, &textRect.w, &textRect.h);
	textRect.x = x;
	textRect.y = FrameWork::Window::instance()->fww_getHeight() - y - textRect.h;

	SDL_RenderCopy(FrameWork::Window::instance()->fww_getWindowRenderer(), textTexture, NULL, &textRect);
