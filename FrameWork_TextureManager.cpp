#include "FrameWork.h"
using FrameWork::TextureManager;
...
unsigned int TextureManager::fwtm_createTextureFromFile(std::string filename)
{
	...
	SDL_Texture* newTexture = SDL_CreateTextureFromSurface(FrameWork::Window::instance()->fww_getWindowRenderer(), newSurface);
...
void TextureManager::fwtm_renderTexture(unsigned int textureID, int x, int y)
{
	...
	destRect.x = x;
	destRect.y = FrameWork::Window::instance()->fww_getHeight() - y - destRect.h;
	SDL_RenderCopy(FrameWork::Window::instance()->fww_getWindowRenderer(), textureArray->at(textureID), NULL, &destRect);
}
...
void TextureManager::fwtm_renderTexture(unsigned int textureID, fw_Rectangle source, fw_Rectangle dest)
{
	...
	int wHeight = FrameWork::Window::instance()->fww_getHeight();

	source.y = tmp - source.y - source.h;

	dest.y = wHeight - dest.h - dest.y;

	SDL_RenderCopy(FrameWork::Window::instance()->fww_getWindowRenderer(), textureArray->at(textureID), &source, &dest);
}
...
void TextureManager::fwtm_renderTexture(fw_Texture* texture, int x, int y)
{
	...
	destRect.x = x;
	destRect.y = FrameWork::Window::instance()->fww_getHeight() - y - destRect.h;
	SDL_RenderCopy(FrameWork::Window::instance()->fww_getWindowRenderer(), texture, NULL, &destRect);
}
...
void TextureManager::fwtm_renderTexture(fw_Texture* texture, fw_Rectangle source, fw_Rectangle dest)
{
	...
	SDL_RenderCopy(FrameWork::Window::instance()->fww_getWindowRenderer(), texture, &source, &dest);
}
